...
VALUES (gen_random_uuid(),
        date_trunc('hour', NOW()) + interval '5 hour', date_trunc('hour', NOW()) + interval '2 hour',
        1, 1, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 4),
       (gen_random_uuid(),
        date_trunc('hour', NOW()) + interval '2 hour', date_trunc('hour', NOW()) + interval '3 hour',
        1, 2, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 4),
       (gen_random_uuid(),
        date_trunc('hour', NOW()) + interval '5 hour', date_trunc('hour', NOW()) + interval '2 hour',
        2, 3, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 4),
       (gen_random_uuid(),
        date_trunc('hour', NOW()) + interval '2 hour', date_trunc('hour', NOW()) + interval '3 hour',
        2, 4, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 10),
       (gen_random_uuid(),
        date_trunc('hour', NOW()) + interval '3 hour', date_trunc('hour', NOW()) + interval '4 hour',
        2, 5, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 10);